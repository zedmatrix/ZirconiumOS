...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- Pass in "systemd" or "sysv" -->
  <xsl:param name="revision" select="'systemd'"/>
  
  <xsl:output method="text" encoding="UTF-8"/>
  <xsl:include href="basename.xsl"/>
  <xsl:include href="print-lines.xsl"/>
  ...
  <xsl:template match="/">
    <xsl:for-each select="//sect1[@id]">
      ...
      <xsl:variable name="pkgtitle" select="sect1info/productname/text()"/>
      <xsl:variable name="pkgversion" select="sect1info/productnumber/text()"/>
      <xsl:variable name="pkgurl" select="sect1info/address/text()"/>

      <xsl:text>title=(</xsl:text><xsl:value-of select="concat($pkgtitle, '-', $pkgversion)"/><xsl:text>)&#10;</xsl:text>
      ...
      <xsl:variable name="preprepare" select=".//screen/userinput[(@remap='pre' and not(@role)) and (not(@revision) or @revision=$revision)]"/>
      <xsl:variable name="prepare"    select=".//screen/userinput[@remap='configure' and not(@role) and (not(@revision) or @revision=$revision)]"/>
      <xsl:variable name="build"      select=".//screen/userinput[@remap='make' and not(@role) and (not(@revision) or @revision=$revision)]"/>
      <xsl:variable name="check"      select=".//screen/userinput[@remap='test' and not(@role) and (not(@revision) or @revision=$revision)]"/>
      <xsl:variable name="install"    select=".//screen/userinput[@remap='install' and not(@role) and (not(@revision) or @revision=$revision)]"/>
      <xsl:variable name="post"       select=".//screen/userinput[@remap='locale-test' or @remap='locale-full']"/>
      <xsl:variable name="final"      select=".//screen/userinput[contains(.,'tzdata')]"/>
...
<!-- Iterate through each listitem -->
      ...
      <xsl:text>&#10;prepare: &#124;&#10;</xsl:text>
      <xsl:for-each select="$prepare | $preprepare">
      ...
      <xsl:for-each select="$build">
        <xsl:text>  </xsl:text> <!-- 2-space indent for YAML block -->
      ...
      <xsl:text>&#10;final: &#124;&#10;</xsl:text>
      <xsl:for-each select="$post | $final">
       <xsl:call-template name="print-lines">
         <xsl:with-param name="text" select="."/>
       </xsl:call-template>
      </xsl:for-each>
    ...
    <xsl:for-each select="$additional//para[contains(., 'Download (HTTP)')]/ulink">
      <xsl:text>&#10; - url: </xsl:text><xsl:value-of select="@url"/><xsl:text>&#10;</xsl:text>
    </xsl:for-each>
