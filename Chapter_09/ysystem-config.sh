...
zmsg "\t Setting Up System Profile Files - $build"
...
export INPUTRC
EOF
[ -f "$build/readline.sh" ] && zmsg " Created: $build/readline.sh"

cat > $build/umask.sh << "EOF"
...
EOF
[ -f "$build/umask.sh" ] && zmsg " Created: $build/umask.sh"

cat > $build/i18n.sh << EOF
# Begin of $build/i18n.sh
...
# End of $build/i18n.sh
EOF
[ -f "$build/i18n.sh" ] && zmsg " Created: $build/i18n.sh"
